...
class _PatientPageState extends State<PatientPage>{
    ...
    void initState(){
        ...
        _timer=Timer.periodic(const Duration(seconds:1),(timer){getPatientData();});
    }
    void dispose(){
        // Cancel the timer when this widget is removed from the widget tree
        _timer.cancel();
        super.dispose();
    }
            ...
            body: Center(
                child: ListView(
                    ...
                    Container(
                        padding: const EdgeInsets.all(8),
                        child: Card(color:Colors.pink,
                        child: ListTile(title: Text("Records",style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold)))
                        ))
                    ])));
}}